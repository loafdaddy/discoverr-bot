...
| Tags | `latest`, semver (e.g. `3.3.2`), and `3.3` (major.minor) |
...
Pin a version tag (e.g. `3.3.2`) for predictable upgrades; use `latest` to always track the newest release.
...
```bash
# edit docker-compose.yml if you pin a version tag, e.g. 3.3.1 → 3.3.2
docker compose pull
docker compose up -d
docker logs -f discoverr
```
...
Your existing `.env` keeps working (3.3.1 → 3.3.2 needs no new vars). Hidden gems now use `TZ` for the two-year cutoff. Do not delete `data/streaming-catalog.json`. You do not need `settings.json` unless you want [extra configuration for posts](#8-extra-configuration-for-posts-optional). Full notes: [docs/RELEASES.md](docs/RELEASES.md).
